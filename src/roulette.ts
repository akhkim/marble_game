...
export class Roulette extends EventTarget {
  ...
  private _gameMode: GameMode = 'individual';
  private _teamCounts: Map<string, number> = new Map(); // total marbles per team
  private _teamFinished: Map<string, number> = new Map(); // finished marbles per team
  private _teamWinners: string[] = []; // teams that have finished all marbles, in order
        ...
        if (this._gameMode === 'team') {
          ...
          if (finished === total && !this._teamWinners.includes(teamName)) {
            ...
            if (this._isRunning && this._teamWinners.length === this._winnerRank + 1) {
              this.dispatchEvent(
                new CustomEvent('goal', { detail: { winner: teamName } }),
              );
              this._winner = marble;
